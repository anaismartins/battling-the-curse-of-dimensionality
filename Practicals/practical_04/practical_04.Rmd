...
The `mnist` object is divided into training and test data and each of them has variables `x` and `y`. `x` is 60000x28x28/10000x28x28 matrix and `y` is a 60000/10000-dimensional vector. `x` has integers between 0 and 255 and `y` has integers between 0 and 9.
...
It has about the same performance as the deep feed forward model (with way fewer parameters).
...
```{r}
pred_alexnet <- class_predict(alexnet, x = mnist$test$x)
(ctab_alexnet <- table(pred = pred_alexnet, true = mnist$test$y))
```

```{r}
sum(diag(ctab_alexnet)) / sum(ctab_alexnet)
```
...
```{r}
pred_vgg16 <- class_predict(vgg16, x = mnist$test$x)
(ctab_vgg16 <- table(pred = pred_vgg16, true = mnist$test$y))
```

```{r}
sum(diag(ctab_vgg16)) / sum(ctab_vgg16)
```
