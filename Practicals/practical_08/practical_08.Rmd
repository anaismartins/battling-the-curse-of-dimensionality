...
```{r}
train_dtm <-
  DocumentTermMatrix(train_docs, control = list(dictionary = freqTerms$term)) %>% tidy()

test_dtm <-
  DocumentTermMatrix(test_docs, control = list(dictionary = freqTerms$term)) %>% tidy()
```

```{r}
dtm_train <- DocumentTermMatrix(train_docs, list(dictionary = features))
dtm_test <- DocumentTermMatrix(test_docs, list(dictionary = features))
...
train_dtm <- data.frame(train_dtm, category = as.factor(doc_cats[match(train_dtm$document, doc_cats$document), "category"]))
train_dtm$category <- as.factor(train_dtm$category)

test_dtm <- data.frame(test_dtm, category = as.factor(doc_cats[match(test_dtm$document, doc_cats$document), "category"]))
test_dtm$category <- as.factor(test_dtm$category)
```

```{r}
dtm_train <- cbind(cat = factor(train_df$Category), dtm_train)
dtm_test <- cbind(cat = factor(test_df$Category), dtm_test)
...
```{r}
model <- svm(formula = category ~ ., data = as.data.frame(train_dtm), type = "C-classification", kernel = "linear")

predict(model)

#predict(model, newdata = as.data.frame(test_dtm))
```
