...
train_tdm <- merge(train_tdm, doc_cats, by = "document")
test_tdm <- merge(test_tdm, doc_cats, by = "document")
```
...
```{r}
model <- svm(formula = category ~ ., data = train_tdm, type = "C-classification", kernel = "linear")

predict(model)

predict(model, newdata = test_tdm)
```
