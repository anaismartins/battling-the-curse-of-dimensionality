...
```{r}
hp_tokens$word <- word_tokenizer(tolower(hp_tokens$word))
...
hp_tokens_bigrams <- hp_tokens %>%
  unnest_tokens(bigram, word, token = "ngrams", n = 2)

hp_tcm2 <- hp_tokens_bigrams %>%
  group_by(book) %>%
  pairwise_count(bigram, sort = TRUE)
```
...
hp_wv_main <- glove$fit_transform(hp_tcm, n_iter = 50, learning_rate = 0.0001)
```
